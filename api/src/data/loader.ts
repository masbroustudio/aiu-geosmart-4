...
export async function getUmkmData(): Promise<UmkmRecord[]> {
  ...
  if (!isMock) {
    try {
      ...
      console.warn("Database query for umkm_dataset returned 0 rows or is invalid. Falling back to CSV.");
    } catch (dbError) {
      console.error("Database query failed for umkm_dataset. Falling back to CSV.", dbError);
    }
  }
...
export async function getUmkmClusteredData(): Promise<UmkmClusteredRecord[]> {
  ...
  if (!isMock) {
    try {
      ...
      console.warn("Database query for umkm_clustered returned 0 rows or is invalid. Falling back to CSV.");
    } catch (dbError) {
      console.error("Database query failed for umkm_clustered. Falling back to CSV.", dbError);
    }
  }
